...
def process_data(intent, is_kb, utts):
    # TODO: process data
    totol_data = []
    dialogues = {}
...
def main(context, intent, is_kb):

    utts = context
    features = [np.array([0]*10)] * len(utts)

    totol_data = process_data(intent, is_kb, utts)
...
def run_demo(context, intent, is_kb):
    fix_random_seed(config.random_seed)
    response = main(context, intent, is_kb)
    return kbs[intent], response
